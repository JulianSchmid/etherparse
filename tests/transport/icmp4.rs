...
mod icmp4_hdr {
    ...
    fn pcap_echo_session() {
        ...
        let request = PacketHeaders::from_ethernet_slice(&request_bytes).unwrap();
        let icmp4 = request.transport.unwrap().icmp4().unwrap();
        ...
        let reply  = PacketHeaders::from_ethernet_slice(&reply_bytes).unwrap();
        let icmp4 = reply.transport.unwrap().icmp4().unwrap();
        match icmp4.icmp_type {
