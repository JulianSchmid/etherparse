...
fn skip_extension() {
    ...
    use std::io::Cursor;
    {
        ...
        let mut cursor = Cursor::new(&buffer);
        assert_matches!(
            Ipv6Header::skip_header_extension(&mut cursor, ICMP),
            Ok(ICMP)
        );
    ...
    }
    {
        ...
        let mut cursor = Cursor::new(&buffer);
        assert_matches!(
            Ipv6Header::skip_header_extension(&mut cursor, IPV6_ROUTE),
            Ok(4)
        );
    ...
    }
    {
        ...
        let mut cursor = Cursor::new(&buffer);
        assert_matches!(
            Ipv6Header::skip_header_extension(&mut cursor, IPV6_FRAG),
            Ok(4)
        );
        assert_eq!(8, cursor.position());
    }
