use crate::{
    err,
    err::Layer,
    icmpv6::{NdpOptionsIterator, NeighborSolicitationPayload},
    LenSource,
};
use core::net::Ipv6Addr;

/// Borrowed payload of a Neighbor Solicitation message (RFC 4861, Section 4.3).
///
...
/// [`crate::Icmpv6Type::NeighborSolicitation`]. This slice starts after that fixed part
/// and contains the fixed `Target Address` followed by options.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct NeighborSolicitationPayloadSlice<'a> {
    slice: &'a [u8],
}

impl<'a> NeighborSolicitationPayloadSlice<'a> {
    /// Length of the fixed payload part (the `Target Address`) in bytes.
    pub const FIXED_PART_LEN: usize = NeighborSolicitationPayload::LEN;
    ...
    pub fn to_payload(&self) -> (NeighborSolicitationPayload, &'a [u8]) {
        (
