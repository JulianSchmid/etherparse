...
impl core::fmt::Display for HeaderSliceError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        use HeaderSliceError::*;
        match self {
            SliceLen(err) => write!(f, "IP Header Error: Not enough data to decode. Length of the slice ({} bytes/octets) is too small to contain an IP header. The slice must at least contain {} bytes/octets.", err.actual_len, err.expected_min_len),
...
mod tests {
    ...
    fn fmt() {
        assert_eq!(
            "IP Header Error: Not enough data to decode. Length of the slice (1 bytes/octets) is too small to contain an IP header. The slice must at least contain 2 bytes/octets.",
            format!(
                "{}",
                SliceLen(
