...
/// Error when decoding an [`crate::IgmpHeader`] from a slice.
#[derive(Clone, Debug, Eq, PartialEq, Hash)]
pub enum HeaderSliceError {
    /// Error when not enough data is present in the slice to decode an
    /// IGMP header.
    Len(LenError),
...
mod tests {
    ...
    fn add_slice_offset() {
        ...
        assert_eq!(
            Content(HeaderError::UnknownType { type_u8: 0xff }).add_slice_offset(200),
            Content(HeaderError::UnknownType { type_u8: 0xff })
        );
    }

    #[test]
    fn debug() {
        let err = HeaderError::UnknownType { type_u8: 0xff };
        assert_eq!(
    ...
    #[test]
    fn clone_eq_hash() {
        let err = Content(HeaderError::UnknownType { type_u8: 0xff });
    ...
    fn source() {
        ...
        .source()
        .is_some());
        assert!(Content(HeaderError::UnknownType { type_u8: 0xff })
            .source()
            .is_some());
    }
}
